...
public class PaymentController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        List<Map<String, String>> promotions = List.of(
            Map.of("id", "percent", "title", "10% Member Discount", "strategyName", "Percentage Discount"),
            Map.of("id", "fixed", "title", "Fixed 50,000đ Off", "strategyName", "Fixed Amount"),
            Map.of("id", "points", "title", "Redeem 10 Loyalty Points", "strategyName", "Points Redemption")
        );
        ...
        if (customer == null) {
            try (EntityManager em = JPAUtil.getEntityManager()) {
                ...
                // DB chưa kết nối được, customer čầu năm null
            }
        }
        ...
        req.getRequestDispatcher("/views/staff/payment.jsp").forward(req, resp);
    }

    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        // 1. Nhận thông tin chiến dịch giảm giá (promo) và phương thức thanh toán (method)
        String promo = req.getParameter("promoStrategy"); 
        String method = req.getParameter("paymentMethod"); // cash, vnpay, card

        // 2. Lấy đối tượng Order từ Session
        HttpSession session = req.getSession();
        Order order = (Order) session.getAttribute("order");
        if (order == null) {
            order = new Order(); // Order rỗng nếu chưa có session
        }

        // STRATEGY PATTERN: Tính subtotal thực tế từ Order
        double subtotal = order.calculateTotal();

        // Tính giảm giá theo chiến lược
        ...
        // Cập nhật State Pattern cho Order thành PendingState
        order.setState(new PendingState());

        // TỔNG QUAN 2: Áp dụng Adapter Pattern & Template Method (Thanh toán)
        PaymentProcessor processor = null;
        
        // Khởi tạo processor thông qua lệnh điều kiện trực tiếp thay vì tạo thêm Factory Pattern
        // VNPAYPaymentProcessor là class đã bọc VNPayAdapter theo Class Diagram
        ...
        // Gọi Template Method (nó sẽ gọi adapter bên trong ExecutePayment nếu là VNPAY)
        Payment paymentResult = processor.processPayment(order);

        // Đổ thông tin dữ liệu đã cập nhật System vào Order
        order.setPaymentStatus(paymentResult.getStatusPayment());

        // 3. Đưa thông tin sang trang xác nhận thành công
        ...
        req.setAttribute("paymentStatus", paymentResult.getStatusPayment().name());
        req.setAttribute("orderState", order.getCurrentState().getClass().getSimpleName());

        req.getRequestDispatcher("/views/staff/success.jsp").forward(req, resp);
    }
}
